...
public:
   ...
   * controlling arm. RobotSide can be
   * either RIGHT or LEFT. arm_pose is a vector of float of size 7 that stores joint angels of all 7 joints in the arm.
   * Time is the relative
   * time for executing the trajectory but it increments for every additional trajectory point. For example: if a
   * trajectory needs to be in
   * pose 1 at 2sec, pose 2 at 5sec, then create 2 objects of this struct one for pose 1 and other for pose 2. pose1
   * object will have time=2
   ...
   * side can be either RIGHT or LEFT. pose is a Pose in task space (world frame) that the hand should move to. time is
   * the total execution time of the trajectory.
   */
