...
void GripperControlInterface::generateGripperMessage(const RobotSide side, const int configuration,
                                                     ihmc_msgs::HandDesiredConfigurationRosMessage& msg)
{
...
{
  return state_informer_->getCurrentPose(side == RobotSide::LEFT ? rd_->getLeftEEFrame() : rd_->getRightEEFrame(), pose,
                                         fixedFrame);
}
...
bool GripperControlInterface::getJointSpaceState(std::vector<double>& joints, RobotSide side)
{
  ...
  double jointPosition =
      state_informer_->getJointPosition(side == RobotSide::LEFT ? rd_->getLeftEEFrame() : rd_->getRightEEFrame());
