 ...
 */
void ArmControlInterface::appendTrajectoryPoint(ihmc_msgs::ArmTrajectoryRosMessage& armMsg, float time,
...
/**
 * @brief ArmControlInterface::nudgeArmLocal will nudge the arm wrt to local end effector coordinates. It is the middle
 * finger in case of Valkyrie arms
 * @param side is the side of the arm
...
bool ArmControlInterface::nudgeArmLocal(const RobotSide side, float x, float y, float z, geometry_msgs::Pose& pose)
{
  ...
  //    std::cout<<"w :"<<value.orientation.w<<" x :"<<value.orientation.x<<" y :"<<value.orientation.y<<" z
  //    :"<<value.orientation.z<<"\n";
  //    moveArmInTaskSpace(side,value, 0.0f);
  pose = value;
  return true;
...
/**
 * @brief ArmControlInterface::nudgeArmPelvis will nudge the arm wrt to pelvis coordinates. z+ is up. x+ is forward. y+
 * is right.
 * @param side is the side of the arm
 * @param x increment in x
 * @param y increment in y
