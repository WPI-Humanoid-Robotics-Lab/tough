...
void* push_back_feedback(void *input_ptr)
{
    ...
    if(left_pose_received)
    {
        ...
        target_pose = transform_pose_simple(&left_pose, VAL_COMMON_NAMES::L_PALM_TF);
        pose_pub->publish(target_pose);
    }
    if(right_pose_received)
    {
        target_pose = transform_pose_simple(&right_pose, VAL_COMMON_NAMES::R_PALM_TF);
        pose_pub->publish(target_pose);
    }
...
int main(int argc, char** argv){
    ...
    *execute_pub = nh_port->advertise<std_msgs::Int16>("/val_pose/execute", 10);
    ...
    {
        ros::Time t = ros::Time::now();
        listener_port->waitForTransform(VAL_COMMON_NAMES::R_PALM_TF,VAL_COMMON_NAMES::WORLD_TF,t, ros::Duration(3.0));
        listener_port->lookupTransform(VAL_COMMON_NAMES::WORLD_TF, VAL_COMMON_NAMES::R_PALM_TF, t,transform);
    }
    catch (tf::TransformException ex)
    {
        ROS_WARN("%s",ex.what());
    ...
    {
        ros::Time t = ros::Time::now();
        listener_port->waitForTransform(VAL_COMMON_NAMES::L_PALM_TF,VAL_COMMON_NAMES::WORLD_TF,t, ros::Duration(3.0));
        listener_port->lookupTransform(VAL_COMMON_NAMES::WORLD_TF, VAL_COMMON_NAMES::L_PALM_TF, t,transform);
    }
    catch (tf::TransformException ex)
    {
        ROS_WARN("%s",ex.what());
