...
void
dplanner::toROSJointTrajectory(const std::vector<descartes_core::TrajectoryPtPtr>& trajectory,
...
void generateDPoints(std::vector<geometry_msgs::Pose> &pose, std::vector<descartes_core::TrajectoryPtPtr>& wayPoints)
{
    for (int i=0; i< pose.size()-1; ++i)
    {
        ...
        // eigen_pose = Eigen::Translation3d(0.0, 0.0, 1.0 + 0.05 * i); //points[i].position.x, points[i].position.y, points[i].position.z);
        //        std::cout << "pose trans " << eigen_pose.translation();
        ROS_INFO("2");
        //        Eigen::Quaternion<double> q(pose[i].orientation.w, pose[i].orientation.x, pose[i].orientation.y, pose[i].orientation.z);
...
void dplanner::plantrajectory(ros::NodeHandle nh, std::vector<geometry_msgs::Pose> &pose, trajectory_msgs::JointTrajectory& traj)
{
