...
void WalkToGoal(geometry_msgs::Pose2D goal) {
    current_state->getCurrentPose(rd->getPelvisFrame(), pelvisPose);
...
void publish_footsteps_cb(const std_msgs::Empty msg){
    ...
    current_state->getCurrentPose(rd->getPelvisFrame(), currPelvisPose);
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber nav_goal_sub    = nh.subscribe("/valkyrie/goal", 1, &nav_goal_cb);
