...
namespace src_qual1_task
{
    ...
    bool getPoseRGB(ImageFrame &img_frame,geometry_msgs::Point &pixelCoordinates,src_perception::MultisenseImage *m_multisenseImagePtr,led_detector::LedPoseColor message, ros::Publisher m_imageRGBXYZpub){
        ...
        // if (countNonZero(img_frame.m_originalImage) < 1)
        // {
            ...
            // Ask Ayush why getting it from the cloud and not the image
            message.color.r = p.r;
            message.color.g = p.g;
            message.color.b = p.b;
        ...
        while(ros::ok())
        {
            ...
            if(m_multisenseImagePtr->giveImage(img_frame.m_originalImage))
            {
                ...
                // if(getLight(img_frame.m_originalImage, pixelCoordinates, m_gradientContours,m_randomGen)){
                    cout<<pixelCoordinates.x<<"     XXXX     "<<pixelCoordinates.y<<endl;
            ...
            if (m_multisenseImagePtr->giveImage(img_frame.m_originalImage))
            {
                ...
                for (int i = 0, j = 0; j < m_gradientContours.size(); j++)
                {
                    ...
                    if (moment.m00)
                    {
                        ...
                        q.setRPY(0, 0, 0);
                        transform.setRotation(q);
                        br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "left_camera_optical_frame", "LED_frame")); //Co-ordinates wrt Head Frame
