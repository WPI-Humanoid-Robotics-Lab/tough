...
void button_detected_callback(const std_msgs::Bool::ConstPtr& msg)
{
    ...
    moveit::planning_interface::MoveGroup group("leftArm");
	...
	if(msg->data == true){
  	...
  	std::vector<double> group_joint_values;
        	...
        	group_joint_values.push_back(1.30);//1.49
        	group_joint_values.push_back(0.0);//0
        	group_joint_values.push_back(0.75);//0
  	group.setJointValueTarget(group_joint_values);

  	  //Planning to a catersian-space goal
  	  // geometry_msgs::Pose target_pose;
    	//   target_pose.orientation.w = 1.0;
  	  // target_pose.position.x = 0.133;
...
int main(int argc, char **argv)
{
  ...
  spinner.start();
  sleep(5.0); // This sleep is ONLY to allow Rviz to come up

  ROS_INFO("raise_arm_move_group");
