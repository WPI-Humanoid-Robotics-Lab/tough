...
bool button_press::pressButton(const armSide side, geometry_msgs::Point &goal, float executionTime)
{
    ...
    if(side == armSide::LEFT){
        ...
        palmToFingerOffset = 0.07;  // offset between middle finger and palm
    ...
    gripper_.controlGripper(RIGHT,GRIPPER_STATE::CLOSE);
    ...
    final.position.z-=0.03; // to press the button
    final.orientation= temp.quaternion;

    waypoints.push_back(final);

    final.position=finalGoal;
    final.position.z+=0.1; // to release the button
    final.orientation= temp.quaternion;
