  ...
  FSM_BGN // begin state machine
  {
    ...
    FSM_STATE(STATE_LEAK_REPAIR)
    {
      ...
      FSM_TRANSITIONS
      {
        ...
        FSM_ON_EVENT("/LEAK_REPAIRED", FSM_NEXT(STATE_WALK_TO_TABLE))
      }
    }
    FSM_STATE(STATE_WALK_TO_TABLE)
    {
      FSM_CALL_TASK(STATE_WALK_TO_TABLE)

      FSM_TRANSITIONS
      {
        FSM_ON_EVENT("/WALK_TO_TABLE_FAILED", FSM_NEXT(STATE_ERROR))
        FSM_ON_EVENT("/WALK_TO_TABLE_EXECUTING", FSM_NEXT(STATE_WALK_TO_TABLE))
        FSM_ON_EVENT("/REACHED_TABLE", FSM_NEXT(STATE_DETECT_FINISH))
      }
    }
