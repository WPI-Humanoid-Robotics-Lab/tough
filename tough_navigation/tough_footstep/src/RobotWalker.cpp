  ...
  : nh_(nh)
{
  ...
  this->footsteps_pub_ = nh_.advertise<ihmc_msgs::FootstepDataListRosMessage>(
      "/ihmc_ros/" + robot_name + "/control/footstep_list", 1, true);
  this->nudgestep_pub_ = nh_.advertise<ihmc_msgs::FootTrajectoryRosMessage>(
      "/ihmc_ros/" + robot_name + "/control/foot_trajectory", 1, true);
  this->loadeff_pub = nh_.advertise<ihmc_msgs::EndEffectorLoadBearingRosMessage>(
      "/ihmc_ros/" + robot_name + "/control/end_effector_load_bearing", 1, true);
  this->abort_footsteps_pub_ =
      nh_.advertise<ihmc_msgs::AbortWalkingRosMessage>("/ihmc_ros/" + robot_name + "/control/abort_walking", 1, true);
  this->footstep_status_ =
      nh_.subscribe("/ihmc_ros/" + robot_name + "/output/footstep_status", 20, &RobotWalker::footstepStatusCB, this);
...
bool RobotWalker::walkNSteps(int numSteps, float xOffset, float yOffset, bool continous, RobotSide startLeg,
                             bool waitForSteps)
{
  ihmc_msgs::FootstepDataListRosMessage list;
  list.default_transfer_duration = transfer_time_;
...
bool RobotWalker::walkNStepsWRTPelvis(int numSteps, float xOffset, float yOffset, bool continous, RobotSide startLeg,
                                      bool waitForSteps)
{
  ihmc_msgs::FootstepDataListRosMessage list;
  list.default_transfer_duration = transfer_time_;
  ...
  if (!continous)
  {
    if (numSteps % 2 == 1)
    {
      list.footstep_data_list.push_back(
          *getOffsetStepWRTPelvis((startLeg + 1) % 2, numSteps * xOffset, numSteps * yOffset));
    }
    if (numSteps % 2 == 0)
    {
...
bool RobotWalker::nudgeFoot(RobotSide side, float distance)
{
  ...
  //        tf_listener_.waitForTransform(TOUGH_COMMON_NAMES::PELVIS_TF,TOUGH_COMMON_NAMES::WORLD_TF,
...
bool RobotWalker::getFootstep(const geometry_msgs::Pose2D& goal, ihmc_msgs::FootstepDataListRosMessage& list)
{
  ...
  start.y = (leftFootPose.position.y + rightFootPose.position.y) /
            2.0f;  // This is required to offset the left foot to get senter of the
