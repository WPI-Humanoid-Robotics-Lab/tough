...
WalkwayGenerator::WalkwayGenerator(ros::NodeHandle& n)
{
  ...
  pointcloudSub_ = nh_.subscribe(PERCEPTION_COMMON_NAMES::ASSEMBLED_LASER_CLOUD_TOPIC, 1,
                                 &WalkwayGenerator::generateWalkwayPoints, this);
...
void WalkwayGenerator::generateWalkwayPoints(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
{
  ...
  pass.setFilterLimits((foot_height - FOOT_GROUND_THRESHOLD - GROUND_THRESHOLD),
                       foot_height - FOOT_GROUND_THRESHOLD + 0.1);
...
void WalkwayGenerator::zAxisLimitFilter(const pcl::PointCloud<pcl::PointXYZ>::Ptr& in_cloud,
...
bool WalkwayGenerator::getLargestCluster(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud)
{
  ...
  ec.setMinClusterSize(
      (int)(0.51 * cloudSize));  // get the cluster with atleast 10% of the total points. this can be 50%
...
double WalkwayGenerator::getCurrentFootHeight(void)
{
  ...
  try
  {
    tf_listener_.waitForTransform(TOUGH_COMMON_NAMES::WORLD_TF, rd_->getLeftFootFrameName(), ros::Time(0),
                                  ros::Duration(3.0));
    tf_listener_.lookupTransform(TOUGH_COMMON_NAMES::WORLD_TF, rd_->getLeftFootFrameName(), ros::Time(0),
                                 transformStamped);
    height_foot = transformStamped.getOrigin().getZ();
    tf_listener_.lookupTransform(TOUGH_COMMON_NAMES::WORLD_TF, rd_->getRightFootFrameName(), ros::Time(0),
                                 transformStamped);
  }
