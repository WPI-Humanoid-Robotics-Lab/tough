 ...
 * @note constructor
 */
MultisensePointCloud::MultisensePointCloud(ros::NodeHandle& nh, const string base_frame,
                                           const string left_camera_opt_frame_tf)
  : nh_(nh)
  , base_frame_(base_frame)
  , left_camera_opt_frame_tf_(left_camera_opt_frame_tf)
...
void MultisensePointCloud::saveToLaserCloud(const sensor_msgs::PointCloud2ConstPtr& msg)
{
  ...
  for (size_t bin = 0; bin < msg->height; bin++)
  {
    ...
    {
      LaserScanPoint* pointT =
          (LaserScanPoint*)&msg->data[bin * msg->width * sizeof(LaserScanPoint) + i * sizeof(LaserScanPoint)];
      LaserPoint T;
      T.x = pointT->x;
      T.y = pointT->y;
      T.z = pointT->z;
...
void MultisensePointCloud::saveToLaserCloudWrtLFoot(const sensor_msgs::PointCloud2ConstPtr& msg)
{
  ...
  laser_cloud_wrt_l_foot_->header.stamp =
      msg->header.stamp.toNSec() / 10e2;  // the stamp in pcl data type is in ms and not ns
...
void MultisensePointCloud::laserCallback(const sensor_msgs::PointCloud2ConstPtr& msg)
{
  ...
  ROS_INFO_ONCE("Laser Cloud: width = %d, height = %d, header = %s, isdense = %d\n", laser_cloud_->width,
                laser_cloud_->height, laser_cloud_->header.frame_id.c_str(), laser_cloud_->is_dense);
}
...
bool MultisensePointCloud::giveLaserCloudForTime(const ros::Time& time, LaserPointCloud::Ptr& out)
{
  ...
  {
    laser_sub_ =
        nh_.subscribe<sensor_msgs::PointCloud2>(laser_topic_.c_str(), 1, &MultisensePointCloud::laserCallback, this);
  ...
  try
  {
    tf_listener.waitForTransform(left_camera_opt_frame_tf_, "left_camera_optical_sweep_fixed",
                                 ros::Time(0) /*time ros::Time().fromNSec(laser_cloud_->header.stamp*10e2)*/,
                                 ros::Duration(1.0));
    tf_listener.lookupTransform(left_camera_opt_frame_tf_, "left_camera_optical_sweep_fixed",
                                ros::Time(0) /*time ros::Time().fromNSec(laser_cloud_->header.stamp*10e2)*/,
                                stamped_tf);
  }
...
void MultisensePointCloud::stereoCallback(const sensor_msgs::PointCloud2ConstPtr& msg)
{
  ...
  ROS_INFO_ONCE("Stereo Cloud: width = %d, height = %d, header = %s, isdense = %d\n", msg->width, msg->height,
                stereo_cloud_->header.frame_id.c_str(), stereo_cloud_->is_dense);
}
/**
...
bool MultisensePointCloud::giveStereoCloud(StereoPointCloud::Ptr& out)
{
  ...
  {
    stereo_sub_ =
        nh_.subscribe<sensor_msgs::PointCloud2>(stereo_topic_.c_str(), 1, &MultisensePointCloud::stereoCallback, this);
    ;
...
bool MultisensePointCloud::giveLaserCloudWrtLFoot(LaserPointCloud::Ptr& out)
{
  ...
  {
    laser_sub_wrt_l_foot_ = nh_.subscribe<sensor_msgs::PointCloud2>(laser_topic_.c_str(), 1,
                                                                    &MultisensePointCloud::laserCallbackWrtLFoot, this);
